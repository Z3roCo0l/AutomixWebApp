...
namespace UnitTest
{
    public class UserServiceTest
    {
        ...
        public async Task Authenticate_ReturnsUser_WhenCredentialsAreValid()
        {
            ...
            await _userService.CreateUser(user.Username, "password", user.Role);
        ...
        public async Task CreateUser_ReturnsCreatedUser()
        {
            ...
            // Act
            var result = await _userService.CreateUser(user.Username, password, user.Role);
        ...
        {
            // Arrange
            var user1 = new User { Username = "admin1", Role = "Admin" };
            var user2 = new User { Username = "admin2", Role = "Admin" };

            _passwordHasherMock.Setup(p => p.HashPassword(It.IsAny<string>()))
                .Returns("hashedpassword");

            await _userService.CreateUser(user1.Username, "password1", user1.Role);
            await _userService.CreateUser(user2.Username, "password2", user2.Role);
